...
from ti.plotting.plots import plot_heatmap
    ...
    for env_id in env_ids:
        ...
        for mname, (mkind, rep) in methods:
            ...
            scal = scalar_scores_from_heat(
                ...
                heat_std,
                layout=layout,
                teacup_inside_mask=teacup_inside_mask if env_spec["ctor"] == "TeacupMaze" else None,
            )
            scal["env"] = env_name
            scal["method"] = mname
            ...
            plot_heatmap(
                heat_mean,
                f"{env_name} | {mname} | Elliptical bonus mean",
                save_path=[
                    os.path.join(fig_dir, f"{env_id}_{mname}_heat_mean.png"),
                    os.path.join(fig_dir, f"{env_id}_{mname}_heat_mean.pdf"),
                ],
            )
            plot_heatmap(
                heat_std,
                f"{env_name} | {mname} | Bonus nuisance sensitivity",
                save_path=[
                    os.path.join(fig_dir, f"{env_id}_{mname}_heat_std.png"),
                    os.path.join(fig_dir, f"{env_id}_{mname}_heat_std.pdf"),
                ],
            )

            if env_spec["ctor"] == "TeacupMaze":
                plot_heatmap(
                    heat_mean,
                    f"{env_name} | {mname} | bonus mean (cup-only)",
                    mask=teacup_inside_mask,
                    save_path=[
                        os.path.join(fig_dir, f"{env_id}_{mname}_heat_mean_cup.png"),
                        os.path.join(fig_dir, f"{env_id}_{mname}_heat_mean_cup.pdf"),
                    ],
                )
                plot_heatmap(
                    heat_std,
                    f"{env_name} | {mname} | nuisance sensitivity (cup-only)",
                    mask=teacup_inside_mask,
                    save_path=[
                        os.path.join(fig_dir, f"{env_id}_{mname}_heat_std_cup.png"),
                        os.path.join(fig_dir, f"{env_id}_{mname}_heat_std_cup.pdf"),
                    ],
                )

    df = pd.DataFrame(all_scalar_rows)
    df.to_csv(os.path.join(table_dir, "elliptical_scalars.csv"), index=False)
