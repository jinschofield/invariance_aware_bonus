...
def run(cfg, fig_id, fig_spec):
    ...
    metrics = [("nuis_acc", "Nuisance acc (low)"), ("nuis_mi", "MI proxy (low)"), ("inv", "Minv (low)")]

    fig, axes = plt.subplots(len(env_ids), len(metrics), figsize=(12, 3 * len(env_ids)))
    if len(env_ids) == 1:
        axes = [axes]
    ...
    for i, env_name in enumerate(env_names):
        ...
        for j, (metric, title) in enumerate(metrics):
            ax = axes[i][j]
            sns.barplot(ax=ax, x="method", y=metric, data=env_rows, color="#4c72b0")
